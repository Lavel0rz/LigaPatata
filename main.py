...
st.set_page_config(page_title="LigaPatata", page_icon="chart_with_upwards_trend", initial_sidebar_state='expanded', layout="wide")
...
if option == 'HOME':
    ...
    with col3:
        st.metric(label='El Victorioso', value=(
                    df_wins2.sort_values(by=['Win%', 'TotalWL'], ascending=False)[:1]['Jugadores'].values[0] + ' ' +
                    df_wins2.sort_values(by=['Win%', 'TotalWL'], ascending=False)[:1]['TotalWL'].values[0]),
                  delta=1, delta_color="normal")
    ...
    st.markdown("<h1 style='text-align: center; color: blue;'>En construccion: Performance Pentagons(PP's)</h1>",
